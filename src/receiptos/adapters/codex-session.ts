...
export type CodexSessionOutput = {
...
function assertCodexSession(source: CodexSessionOutput) {
  if (source.schema !== "codex.session.v0") {
    throw new Error(`Expected codex.session.v0, got ${String(source.schema)}`)
  }
  if (!source.session_id) throw new Error("codex.session.v0 missing session_id")
  if (!source.task?.title || !source.task?.prompt_summary) {
    throw new Error("codex.session.v0 missing task title or prompt_summary")
  }
  if (source.runtime?.producer_id !== "codex") {
    throw new Error("codex.session.v0 producer_id must be codex")
  }
  if (source.runtime?.generated_by !== "codex.session.v0") {
    throw new Error("codex.session.v0 generated_by must be codex.session.v0")
  }
  for (const event of source.events) {
    if (!event.type) throw new Error("codex.session.v0 event missing type")
    if (event.type === "tool_use" && typeof event.exit_code === "number" && !event.command_summary) {
      throw new Error("codex.session.v0 exec-like tool_use with exit_code requires command_summary")
    }
  }
}

function deriveAllowedActions(events: CodexSessionOutput["events"]): string[] {
...
export function normalizeCodexSessionOutput(source: CodexSessionOutput): HandoffEvidence {
  assertCodexSession(source)
  ...
  if (toolEvents.length === 0) {
    throw new Error("codex.session.v0 contains no tool_use events")
  }
