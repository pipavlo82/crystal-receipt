#!/usr/bin/env python3
"""Independent verifier-challenge-chronicle-proof-root-mismatch-rejected-v0 auditor.
...
PACKAGE = ROOT / "conformance/verifier-challenge-chronicle-proof-root-mismatch-rejected-v0"
...
def apply_mutation(baseline: dict, mutation: dict) -> dict:
    ...
    path = mutation["path"]
    if path != ["proof_object", "receipt_root"]:
        raise ValueError("mutation path must be proof_object.receipt_root only")
    ...
    if proof_object.get("receipt_root") != mutation["from"]:
        raise ValueError("baseline proof_object.receipt_root does not match mutation.from")
    proof_object["receipt_root"] = mutation["to"]
    return challenged
...
def evaluate_vector_semantics(vector: dict) -> None:
    ...
    assert expected["proof_object_receipt_root_changed_only"] is True
    assert expected["cross_object_root_mismatch_blocks_admission"] is True
    assert expected["failure_class_exact"] == "cross_object_inconsistency"
    assert expected["reason_code_exact"] == "proof_root_mismatch"
    ...
    assert challenged_admission["failure"]["failure_class"] == "cross_object_inconsistency"
    assert challenged_admission["failure"]["reason_code"] == "proof_root_mismatch"

    assert receipt_root_control["ok"] is True
    assert receipt_root_control["receipt_root"] == receipt_root_control["recomputed_root"]
    assert receipt_root_control["receipt_root"] == baseline_admission["value"]["receipt_root"]

    mutation = vector["mutation"]
    assert mutation["from"] == "0x687dc5c00d9241469138bb1c17a06af1b8713b0f84663b55e11d476f4171a6bc"
    assert mutation["to"] == "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"

    classification = vector["field_classification"]
    assert classification["unchanged_handoff_evidence"] is True
    assert classification["cross_object_admission_binding"] == ["proof_object.receipt_root"]
    assert "evidence" in classification["excluded_from_mutation"]
    assert "anchor.receipt_root" in classification["excluded_from_mutation"]
...
def audit_package() -> dict:
    ...
    vector = json.loads((VECTORS / "V-CHRONICLE-PROOF-ROOT-MISMATCH.json").read_text(encoding="utf-8"))
    assert vector["vector_id"] == "V-CHRONICLE-PROOF-ROOT-MISMATCH"
    assert vector["execution_class"] == "production-admission-binding"
    evaluate_vector_semantics(vector)

    expected_row = (
        f"V-CHRONICLE-PROOF-ROOT-MISMATCH\t"
    ...
    assert (
        git_index_blob_oid(str(profile["portable_proof_object_module_path"]))
    ...
    assert baseline_input["proof_object"]["receipt_root"] == vector["mutation"]["from"]
    assert challenged_input["proof_object"]["receipt_root"] == vector["mutation"]["to"]

    baseline_without_proof_root = json.loads(json.dumps(baseline_input))
    challenged_without_proof_root = json.loads(json.dumps(challenged_input))
    baseline_without_proof_root["proof_object"] = {
        k: v
        for k, v in baseline_without_proof_root["proof_object"].items()
        if k != "receipt_root"
    }
    challenged_without_proof_root["proof_object"] = {
        k: v
        for k, v in challenged_without_proof_root["proof_object"].items()
        if k != "receipt_root"
    }
    assert baseline_without_proof_root == challenged_without_proof_root

    return {
        "auditor": "python-independent-verifier-challenge-chronicle-proof-root-mismatch-rejected-v0",
        "vector_count": 1,
