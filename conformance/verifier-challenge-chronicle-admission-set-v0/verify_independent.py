#!/usr/bin/env python3
"""Independent verifier-challenge-chronicle-admission-set-v0 aggregate index auditor.

Reads only frozen aggregate and referenced child package files.
Imports no ReceiptOS production code.
Does not execute tryCreateChronicleEntryV0 or re-derive child vector semantics.
"""
...
PACKAGE = ROOT / "conformance/verifier-challenge-chronicle-admission-set-v0"
...
def verify_child_package(child: dict) -> None:
    ...
    assert child["execution_class"] == "production-admission-binding", package_path
...
def audit_package() -> dict:
    ...
    assert contract["set_id"] == "verifier-challenge-chronicle-admission-set-v0"
    ...
    subject = contract["subject_admission_verifier"]
    assert subject["entrypoint"] == "tryCreateChronicleEntryV0"
    ...
    assert aggregate["execution_class_counts"] == {"production-admission-binding": 3}

    identity_hash = child_identity_set_sha256(children)
    assert identity_hash == aggregate["child_identity_set_sha256"]

    return {
        "auditor": "python-independent-verifier-challenge-chronicle-admission-set-v0",
