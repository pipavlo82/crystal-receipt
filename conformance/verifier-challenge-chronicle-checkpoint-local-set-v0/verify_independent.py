#!/usr/bin/env python3
"""Independent verifier-challenge-chronicle-checkpoint-local-set-v0 aggregate index auditor.

Reads only frozen aggregate and referenced child package files.
Imports no ReceiptOS production code.
Does not execute verifyChronicleCheckpointV0 or re-derive child vector semantics.
"""
...
PACKAGE = ROOT / "conformance/verifier-challenge-chronicle-checkpoint-local-set-v0"
...
def verify_child_package(child: dict) -> None:
    ...
    assert child["execution_class"] == "production-checkpoint-local-binding", package_path
...
def audit_package() -> dict:
    ...
    assert contract["set_id"] == "verifier-challenge-chronicle-checkpoint-local-set-v0"
    ...
    subject = contract["subject_local_checkpoint_verifier"]
    assert subject["entrypoint"] == "verifyChronicleCheckpointV0"
    assert git_index_blob_oid(subject["module_path"]) == subject["git_blob_oid"]
    assert manifest["subject_local_checkpoint_verifier_git_blob_oid"] == subject["git_blob_oid"]

    normative = contract["normative_spec_identity"]
    assert git_index_blob_oid(normative["repository_path"]) == normative["git_blob_oid"]
    assert manifest["normative_spec_git_blob_oid"] == normative["git_blob_oid"]
    ...
    assert aggregate["child_count"] == 2
    assert aggregate["vector_count"] == 0
    assert aggregate["child_vector_count"] == 2
    assert aggregate["execution_class_counts"] == {"production-checkpoint-local-binding": 2}
    assert manifest["aggregate_vector_count"] == 0
    assert manifest["child_vector_count"] == 2
    ...
    return {
        "auditor": "python-independent-verifier-challenge-chronicle-checkpoint-local-set-v0",
