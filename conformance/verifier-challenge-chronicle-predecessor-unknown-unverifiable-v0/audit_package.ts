/** Independent verifier-challenge-chronicle-predecessor-unknown-unverifiable-v0 package audit. No production imports. */
...
const PACKAGE = "conformance/verifier-challenge-chronicle-predecessor-unknown-unverifiable-v0"
...
function evaluateVectorSemantics(vector: J): void {
  ...
  assert(expected.challenged_evaluation_state === "unverifiable", "challenged evaluation state")
  assert(expected.challenged_verdict === null, "challenged verdict")
  assert(expected.challenged_relation === null, "challenged relation")
  assert(expected.challenged_reason_code === "predecessor_unknown", "challenged reason code")
  ...
  assert(expected.current_is_non_genesis === true, "current non-genesis")
  assert(expected.predecessor_argument_removed_only === true, "predecessor removed only")
  assert(expected.missing_predecessor_blocks_evaluation === true, "missing blocks evaluation")
  assert(expected.missing_predecessor_does_not_imply_invalid === true, "missing not invalid")
  assert(expected.predecessor_ref_gate_not_reached === true, "ref gate not reached")
  assert(expected.sequence_gate_not_reached === true, "sequence gate not reached")
  ...
  assert(profile.challenged_first_failure_gate === "predecessor_unknown", "first failure gate")

  const classification = vector.field_classification as J
  assert(classification.predecessor_argument_removed_only === true, "argument removed only")
...
export function auditPackage(): J {
  ...
  const vector = readJson(`${PACKAGE}/vectors/V-CHRONICLE-PREDECESSOR-UNKNOWN.json`)
  assert(vector.execution_class === "production-continuity-binding", "execution class")
  evaluateVectorSemantics(vector)
  const resultRow = `V-CHRONICLE-PREDECESSOR-UNKNOWN\t${shaBytes(Buffer.from(canonicalExpected(vector.expected), "utf8"))}\n`
  ...
  assert(challengedPair.predecessor === null, "challenged predecessor null")
  assert(JSON.stringify(baselinePair.current) === JSON.stringify(challengedPair.current), "current unchanged")
  assert(baselinePair.predecessor !== null, "baseline predecessor present")
  assert((vector.substitution as J).operation === "remove_predecessor_argument", "substitution operation")
  assert((vector.substitution as J).to === null, "substitution to null")

  const current = baselinePair.current as J
  assert(Number(current.sequence) > 0, "current non-genesis")

  const controls = vector.local_verification_controls as J
  for (const key of ["current", "baseline_predecessor"]) {
  ...
  return {
    auditor: "typescript-independent-verifier-challenge-chronicle-predecessor-unknown-unverifiable-v0",
