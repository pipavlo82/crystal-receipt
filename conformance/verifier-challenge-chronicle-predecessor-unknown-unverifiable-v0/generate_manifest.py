...
PKG = ROOT / "conformance/verifier-challenge-chronicle-predecessor-unknown-unverifiable-v0"

members = [
    "conformance/verifier-challenge-chronicle-predecessor-unknown-unverifiable-v0/SPEC.md",
...
vector = json.loads((PKG / "vectors/V-CHRONICLE-PREDECESSOR-UNKNOWN.json").read_text(encoding="utf-8"))
exp = json.dumps(vector["expected"], ensure_ascii=False, separators=(",", ":"), sort_keys=True)
result_hash = hashlib.sha256(
    f"V-CHRONICLE-PREDECESSOR-UNKNOWN\t{hashlib.sha256(exp.encode()).hexdigest()}\n".encode()
).hexdigest()

contract = {
    "schema": "verifier_challenge_chronicle_predecessor_unknown_unverifiable_contract.v0",
    "package_version": "verifier-challenge-chronicle-predecessor-unknown-unverifiable-v0",
    "profile_id": "verifier-challenge-chronicle-predecessor-unknown-unverifiable-v0",
    "challenge_id": "predecessor_unknown_unverifiable",
    "authority": {
        "normative_spec": "conformance/verifier-challenge-chronicle-predecessor-unknown-unverifiable-v0/SPEC.md",
    ...
    "forbidden_semantics": [
        ...
        "genesis_positive_classification",
        "chronicle_admission",
    ...
    "vector_inventory": ["V-CHRONICLE-PREDECESSOR-UNKNOWN"],
    ...
    "vector_execution_classes": {
        ...
        "vectors": {
            "V-CHRONICLE-PREDECESSOR-UNKNOWN": {"execution_class": "production-continuity-binding"},
        },
    },
    "independence_scope": {
        "package_identity": "Python and TypeScript auditors recompute member and expected-result digests without production imports.",
        "challenge_semantics": "Independent verifier validates substitution scope, gate-order relation, and encoded epistemic continuity results from frozen spec.",
...
files.append({"path": "conformance/verifier-challenge-chronicle-predecessor-unknown-unverifiable-v0/contract.json", "sha256": contract_hash})
files.sort(key=lambda item: item["path"])
file_rows = [f'{item["path"]}\t{item["sha256"]}\n' for item in files]
fixture_hash = hashlib.sha256("".join(file_rows).encode()).hexdigest()

manifest = {
    "schema": "verifier_challenge_chronicle_predecessor_unknown_unverifiable_fixture_manifest.v0",
    "package_version": "verifier-challenge-chronicle-predecessor-unknown-unverifiable-v0",
