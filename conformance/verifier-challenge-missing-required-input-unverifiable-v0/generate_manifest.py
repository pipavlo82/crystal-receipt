...
PKG = ROOT / "conformance/verifier-challenge-missing-required-input-unverifiable-v0"

members = [
    "conformance/verifier-challenge-missing-required-input-unverifiable-v0/SPEC.md",
...
vector = json.loads((PKG / "vectors/V-MISSING-REQUIRED-INPUT.json").read_text(encoding="utf-8"))
exp = json.dumps(vector["expected"], ensure_ascii=False, separators=(",", ":"), sort_keys=True)
result_hash = hashlib.sha256(
    f"V-MISSING-REQUIRED-INPUT\t{hashlib.sha256(exp.encode()).hexdigest()}\n".encode()
).hexdigest()

contract = {
    "schema": "verifier_challenge_missing_required_input_unverifiable_contract.v0",
    "package_version": "verifier-challenge-missing-required-input-unverifiable-v0",
    "profile_id": "verifier-challenge-missing-required-input-unverifiable-v0",
    "challenge_id": "missing_required_input_unverifiable",
    "authority": {
        "normative_spec": "conformance/verifier-challenge-missing-required-input-unverifiable-v0/SPEC.md",
    ...
    "forbidden_semantics": [
        ...
        "general_malformed_input_validation",
        "schema_validation",
        "admission_semantics",
        "timing_semantics",
    ],
    ...
    "vector_inventory": ["V-MISSING-REQUIRED-INPUT"],
    ...
    "vector_execution_classes": {
        ...
        "vectors": {
            "V-MISSING-REQUIRED-INPUT": {"execution_class": "production-verifier-binding"},
        },
    },
    "independence_scope": {
        "package_identity": "Python and TypeScript auditors recompute member and expected-result digests without production imports.",
        "challenge_semantics": "Independent verifier validates mutation scope, field classification, and encoded missing-input unverifiability relation from frozen spec.",
...
files.append({"path": "conformance/verifier-challenge-missing-required-input-unverifiable-v0/contract.json", "sha256": contract_hash})
files.sort(key=lambda item: item["path"])
file_rows = [f'{item["path"]}\t{item["sha256"]}\n' for item in files]
fixture_hash = hashlib.sha256("".join(file_rows).encode()).hexdigest()

manifest = {
    "schema": "verifier_challenge_missing_required_input_unverifiable_fixture_manifest.v0",
    "package_version": "verifier-challenge-missing-required-input-unverifiable-v0",
