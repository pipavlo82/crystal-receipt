/** Independent verifier-challenge-missing-required-input-unverifiable-v0 package audit. No production imports. */
...
const PACKAGE = "conformance/verifier-challenge-missing-required-input-unverifiable-v0"
...
function evaluateVectorSemantics(vector: J): void {
  ...
  assert(expected.missing_input_cannot_establish_validity === true, "missing input non-elevation")
  assert(baseline.ok === true && baseline.receipt_root === baseline.recomputed_root, "baseline validity")
  assert(challenged.ok === false && challenged.receipt_root === null && challenged.recomputed_root === null, "challenged unverifiable")
  const mutation = vector.mutation as J
  assert(mutation.operation === "set" && JSON.stringify(mutation.path) === '["anchor","receipt_root"]', "mutation")
  assert(mutation.to === null, "mutation to null")
  const classification = vector.field_classification as J
  assert(JSON.stringify(classification.required_decisive_operand) === '["anchor.receipt_root"]', "required operand")
  assert(classification.absent_operand_prevents_comparison === true, "absent operand guard")
  const profile = vector.receipt_root_profile as J
  assert(profile.missing_operand_behavior === "return_ok_false_null_roots_without_throw", "fail-closed behavior")
}
...
export function auditPackage(): J {
  ...
  const vector = readJson(`${PACKAGE}/vectors/V-MISSING-REQUIRED-INPUT.json`)
  evaluateVectorSemantics(vector)
  const resultRow = `V-MISSING-REQUIRED-INPUT\t${shaBytes(Buffer.from(canonicalExpected(vector.expected), "utf8"))}\n`
  ...
  return {
    auditor: "typescript-independent-verifier-challenge-missing-required-input-unverifiable-v0",
