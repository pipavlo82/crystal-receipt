/** Independent verifier-challenge-chronicle-capsule-label-inconsistent-rejected-v0 package audit. No production imports. */
...
const PACKAGE = "conformance/verifier-challenge-chronicle-capsule-label-inconsistent-rejected-v0"
...
function evaluateVectorSemantics(vector: J): void {
  ...
  assert(expected.proof_object_receipt_root_unchanged === true, "proof root unchanged")
  assert(expected.capsule_stored_root_unchanged === true, "stored unchanged")
  assert(expected.capsule_computed_root_unchanged === true, "computed unchanged")
  assert(expected.capsule_match_changed_only === true, "match only change")
  assert(expected.capsule_status_unchanged === true, "status unchanged")
  assert(expected.verifier_result_unchanged === true, "verifier result unchanged")
  assert(expected.proof_object_id_unchanged === true, "id unchanged")
  assert(expected.proof_ref_unchanged === true, "ref unchanged")
  assert(expected.cross_object_gates_pass === true, "cross object gates pass")
  assert(expected.reported_state_blocks_admission === true, "reported state blocks")
  assert(expected.failure_class_exact === "reported_state_inconsistency", "failure class")
  assert(expected.reason_code_exact === "capsule_label_inconsistent", "reason code")
  ...
  assert(
    (challengedAdmission.failure as J).failure_class === "reported_state_inconsistency",
    "challenged failure class",
  )
  assert((challengedAdmission.failure as J).reason_code === "capsule_label_inconsistent", "challenged reason code")
  ...
  assert(
    JSON.stringify(mutation.path) === '["proof_object","evidence_capsule","receipt_root","match"]',
    "mutation path",
  )
  assert(mutation.from === true && mutation.to === false, "mutation values")

  const profile = vector.admission_profile as J
  assert(profile.challenged_first_failure_gate === "capsule_label_inconsistent", "first failure gate")
  ...
  assert(
    JSON.stringify(classification.reported_state_admission_binding) ===
...
export function auditPackage(): J {
  ...
  const vector = readJson(`${PACKAGE}/vectors/V-CHRONICLE-CAPSULE-LABEL-INCONSISTENT.json`)
  assert(vector.execution_class === "production-admission-binding", "execution class")
  evaluateVectorSemantics(vector)
  const resultRow = `V-CHRONICLE-CAPSULE-LABEL-INCONSISTENT\t${shaBytes(Buffer.from(canonicalExpected(vector.expected), "utf8"))}\n`
  ...
  assert(
    gitIndexBlobOid(String((vector.identity_control_authority as J).module_path)) ===
      (vector.identity_control_authority as J).git_blob_oid,
  ...
  return {
    auditor: "typescript-independent-verifier-challenge-chronicle-capsule-label-inconsistent-rejected-v0",
