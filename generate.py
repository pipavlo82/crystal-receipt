...
def generate_crystal_spec(receipt_hash: str) -> Dict:
    digest, seed = seed_from_hash(receipt_hash)
    rng = random.Random(seed)

    palette = PALETTES[int(digest[16:18], 16) % len(PALETTES)]
...
def write_outputs(receipt_hash: str, out_dir: Path) -> Dict:
    spec = generate_crystal_spec(receipt_hash)
    svg = render_svg(spec)
...
def main() -> None:
    ...
    parser.add_argument("--hash", required=True, dest="receipt_hash", help="Receipt hash string")
    parser.add_argument("--out", required=True, help="Output directory")
    args = parser.parse_args()
    spec = write_outputs(args.receipt_hash, Path(args.out))
    print(json.dumps({"ok": True, "out": args.out, "seed": spec["seed"], "sha256": spec["sha256"]}, indent=2))
