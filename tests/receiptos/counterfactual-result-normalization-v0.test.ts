...
function assertObservation(obs: CounterfactualObservationV0, cls: CounterfactualObservationClassV0) {
  ...
  expect(obs.source_validity_effect).toBe("none")
}
...
describe("counterfactual result normalization v0", () => {
  ...
  test("verifyHandoff raw {ok:false} cannot encode observed_not_validated challenge semantics", () => {
    ...
    // Limitation: observed_not_validated is not a distinct observation_class from affirmative success.
    expect(observed.expected.observation_cannot_establish_validity).toBe(true)
    expect(nObserved.observation_class).toBe("affirmative")
  })

  test("Chronicle admission admitted → affirmative with source_validity_effect none", () => {
    const vector = readJson(
    ...
    expect(admitted.source_validity_effect).toBe("none")
  ...
  test("Chronicle admission proof-root mismatch → rejected (local admission, not source invalidity)", () => {
    ...
    expect(fromExpected.source_validity_effect).toBe("none")
  ...
  test("host/runtime non-collapse: thrown Error and invalid inputs are not semantic observations", () => {
    ...
    expect(() => normalizeChronicleContinuityResult({ evaluation_state: "weird" } as never)).toThrow(
      NormalizationContractError,
    )
