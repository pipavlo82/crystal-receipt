...
import { auditPackage } from "../../conformance/verifier-challenge-chronicle-proof-object-id-invalid-rejected-v0/audit_package"
...
import { deriveProofObjectId } from "../../src/receiptos/capsule/portable-proof-object-v0"
import { verifyHandoffReceiptRoot } from "../../src/receiptos/verify/verify-receipt"

const root = resolve(import.meta.dir, "../..")
const pkg = "conformance/verifier-challenge-chronicle-proof-object-id-invalid-rejected-v0"
const vector = JSON.parse(
  readFileSync(resolve(root, `${pkg}/vectors/V-CHRONICLE-PROOF-OBJECT-ID-INVALID.json`), "utf8"),
)
...
function applyMutation(input: AdmissionInput): AdmissionInput {
  ...
  expect(challenged.proof_object.proof_object_id).toBe(vector.mutation.from)
  challenged.proof_object.proof_object_id = vector.mutation.to
  return challenged
}

function withoutProofObjectId(input: AdmissionInput) {
  const cloned = structuredClone(input)
  const { proof_object_id: _ignored, ...proofWithoutId } = cloned.proof_object
  return {
    ...cloned,
    proof_object: proofWithoutId,
  }
}

describe("verifier-challenge-chronicle-proof-object-id-invalid-rejected-v0 package", () => {
  ...
  test("frozen vector binds tryCreateChronicleEntryV0 proof_object_id identity rejection", async () => {
    const baselineInput = loadBaselineInput()
    expect(baselineInput.proof_object.proof_object_id).toBe(vector.mutation.from)
    ...
    expect(baselineInput.evidence.anchor.receipt_root).toBe(VERIFIED_ROOT)
    ...
    expect(challengedInput.proof_object.evidence_capsule).toEqual(baselineInput.proof_object.evidence_capsule)
    expect(withoutProofObjectId(challengedInput)).toEqual(withoutProofObjectId(baselineInput))
    expect(challengedInput.proof_object.proof_object_id).toBe(vector.mutation.to)

    const capsuleRoot = challengedInput.proof_object.evidence_capsule.receipt_root
    expect(challengedInput.proof_object.receipt_root).toBe(VERIFIED_ROOT)
