/**
 * Public evidence for the real independent-authority Rekor v1 run.
 *
 * Offline: frozen public E0-record + public Rekor entries.
 * Does not read private-run payloads. Does not mint production
 * evaluateProductionIndependentGrounding PROVEN.
 */
...
const SCHEMA = "tsei-invariant-discrimination-v0.rekor-v1-real-run-public-evidence.v0"
const CASE_IDS = ["c01", "c02", "c03", "c04", "c05", "c06", "c07", "c08", "c09", "c10", "c11", "c12"] as const
const EXPECTED_SETS: Record<(typeof CASE_IDS)[number], readonly string[]> = {
  c01: [],
  c02: ["I_A"],
  c03: [],
  c04: ["I_A", "I_B"],
...
describe("TSEI real-run public evidence v0", () => {
  ...
  test("schema, canonical bytes, and derived status exactness", () => {
    ...
    expect(derived.mechanical_exact_set_relation).toBe("AGREES")
    expect(derived.public_evidence_status).toBe("REKOR_V1_E0_LT_E1_LT_E2_VERIFIED_EXACT_SET_AGREES")
    expect(parsed.public_reproducibility).toBe("NOT_FULLY_PUBLICLY_REPRODUCIBLE_PRIVATE_PAYLOADS_UNPUBLISHED")
  })

  test("12/12 exact-set agreement and non-gating ambiguity cases", () => {
    const parsed = asRecord(loadJson("public-evidence.json"))
    const agreement = asRecord(parsed.exact_set_agreement)
    expect(Object.keys(agreement).sort()).toEqual([...CASE_IDS])
    for (const id of CASE_IDS) {
      expect(agreement[id]).toEqual([...EXPECTED_SETS[id]])
    }
  ...
  test("ordering, identity, and commitment bindings", () => {
    const parsed = asRecord(loadJson("public-evidence.json"))
    ...
    expect(order.strict).toBe("E0_lt_E1_lt_E2")
    expect(order.global_log_indexes).toEqual([2533202771, 2534488743, 2535402205])
  ...
  test("frozen E1/E2 entries bind published digests without private payloads", () => {
