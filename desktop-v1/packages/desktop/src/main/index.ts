...
const APP_NAMES: Record<string, string> = {
  dev: "Crystal Receipt Desktop Dev",
  beta: "Crystal Receipt Desktop Beta",
  prod: "Crystal Receipt Desktop",
}
const APP_IDS: Record<string, string> = {
  dev: "network.cyphes.crystal-receipt.dev",
  beta: "network.cyphes.crystal-receipt.beta",
...
const main = Effect.gen(function* () {
  ...
  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "network.cyphes.crystal-receipt.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `stealth-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Crystal Receipt Desktop Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("stealth://") || arg.startsWith("opencode://"))
    if (urls.length) {
