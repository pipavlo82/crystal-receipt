...
| Tamper / missing-field negatives | this repo, main — src/receiptos/fixtures/invalid/ | Current | Six negative cases (malformed anchor result, mismatched root, wrong chain-id, wrong network, malformed txhash, missing required anchor-result field); agreement is on rejection semantics; expected rejection semantics pinned in fixtures/invalid/EXPECTED.md; tri-state verdict mapping locked in EXPECTED.md (5/5 gate agreement) |
